...
class HolderChatUser(rootView: ViewGroup, private val presenter: ChatPresenter) : BaseViewHolder<Message>(rootView, R.layout.item_chat_user) {
    ...
    override fun bind(item: Message) {
        with(itemView) {
            ...
            tvNameUserMeet.typeface = boldTypeFace
            ...
            llName.visibility = if (item.photoUrl != null) View.GONE else View.VISIBLE

            if (item.onTopic) {
                llMain.setBackgroundResource(R.drawable.background_chat_on_topic)
                tvOnTopic.visibility = View.VISIBLE
            } else {
                llMain.setBackgroundResource(R.drawable.background_chat_user)
                tvOnTopic.visibility = View.GONE
            }
            imgAvatar.visibility = if (item.showAvatar) {
                imgAvatar.loadImage(item.user.image)
                imgAvatar.setOnClickListener { presenter.onUserShow(item.user) }
                View.VISIBLE
            } else {
            ...
            if (item.text.isNotBlank()) {
                ...
                tvMessage.text = SpannableString("${item.text}00:00")
                        .apply { setSpan(ForegroundColorSpan(Color.TRANSPARENT), item.text.length, item.text.length + 5, 0) }
            ...
            if (item.meet != null) {
                ...
                tvNameUserMeet.text = item.meet.name
